...
Auth    : Authorization: PBSAPIToken=user@realm!name=value
...
class PBSClient:
    ...
    async def list_snapshots(self, datastore: str, vmid: int | None = None) -> list[dict]:
        """
        Liste les snapshots dans un datastore.
        Optionnellement filtrés par vmid (backup-id au format 'vm/100').
        """
        params: dict = {}
        if vmid is not None:
